...
void hal_ant_switch (uint8_t val);
...
uint8_t hal_sleep (uint8_t type, uint32_t targettime);
 ...
 */
uint8_t hal_getBattLevel (void);

/*
 * set current battery level
 */
void hal_setBattLevel (uint8_t level);
...
uint8_t* hal_joineui (void);
uint8_t* hal_deveui (void);
uint8_t* hal_nwkkey (void);
uint8_t* hal_appkey (void);
uint8_t* hal_serial (void);
uint32_t  hal_region (void);
uint32_t  hal_hwid (void);
uint32_t  hal_unique (void);

uint32_t hal_dnonce_next (void);
